...
class SplashScreen:
    def __init__(self):
        ...
        # Add loading text
        tk.Label(frame, text="EEG Recorder", font=('Helvetica', 16, 'bold')).pack(pady=20)
...
class EEGRecorder:
    def __init__(self, root):
        ...
        self.duration_var = tk.StringVar(value="Duration: 0 seconds")
        ...
        # Duration
        ttk.Label(info_frame, textvariable=self.duration_var).grid(row=0, column=0, padx=5)
    ...
    def show_about(self):
        messagebox.showinfo("About", "EEG Recorder\nVersion 1.0\nA simple EEG recording application.")
    ...
    def update_duration(self):
        ...
        if self.recording and self.recording_start_time:
            ...
            self.duration_var.set(f"Duration: {duration} seconds")
            self.root.after(1000, self.update_duration)

    def start_recording(self):
        if not self.port_var.get():
            messagebox.showerror("Error", "EEG optical dongle not connected.\nPlease connect the device and wait for it to be detected.")
            return
    ...
    def stop_recording(self):
        if self.recording:
            ...
            self.duration_var.set("Duration: 0 seconds")
            self.sample_rate_var.set("Sample Rate: -- Hz")
